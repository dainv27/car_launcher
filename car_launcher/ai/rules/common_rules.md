...
### State Management
* **Riverpod (primary):** This project uses `flutter_riverpod` with `@riverpod`
  code generation for ALL new app state. See `ai/rules/architecture_rules.md`
  for the canonical patterns (`@riverpod` classes, `AsyncValue`, `ref.watch` /
  `ref.read` / `ref.listen`, `ConsumerWidget`).
* **Legacy:** Older files still use `ChangeNotifier` + `auto_route` + `flash`.
  Do NOT introduce new code with that pattern — migrate it to Riverpod when you
  touch it.
* **Streams:** Use `Streams` for sequences of async events (e.g. vehicle
  tracking). Dispose subscriptions in the notifier's `dispose()`.
* **Futures / AsyncValue:** Model async app state as `AsyncValue<T>` inside
  Riverpod notifiers; render with `when(data:, loading:, error:)`. Never unwrap
  with `!`.
* **Local/ephemeral state:** Use `StatefulWidget` / `ValueNotifier` only for
  truly local UI state (animation controllers, form field controllers) that is
  not shared across the tree.
* **Dependency Injection:** Riverpod handles DI for app state via `ref.read`.
  `get_it` is used only for non-Riverpod singletons (API clients, DB helpers,
  native bridge). See `architecture_rules.md` §5.
